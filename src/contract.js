...
class Contract {
   ...
   */
  constructor(protocol, address, abi) {
  ...
  async call(methodName, params) {
    try {
      ...
      let result = await this.protocol.rawCall('callcontract', [this.address, data, senderAddress]);
  ...
  async send(methodName, params) {
    try {
      ...
      const result = await this.protocol.rawCall('sendtocontract', [
        this.address,
        data,
        amt,
        limit,
